...
const getMineEmissionModel = (mineName) => {
  const modelName = `Emission_${mineName}`;
  const exactCollectionName = `emission_${mineName}`;
  return mongoose.models[modelName] || mongoose.model(modelName, dailyEmissionSchema, exactCollectionName);
};
