...
// DELETE /api/emissions - Delete emission records for a specific mine
router.delete('/', async (req, res) => {
  try {
    const { mineId, recordIds } = req.body;
    
    if (!mineId || !recordIds || !Array.isArray(recordIds)) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    ...
    // Delete all records whose IDs are in the array
    await MineEmission.deleteMany({ _id: { $in: recordIds } });
    
    res.status(200).json({ success: true, message: `${recordIds.length} records deleted.` });
  } catch (err) {
